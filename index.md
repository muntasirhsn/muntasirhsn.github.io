...
This project showcases a multistep time-series forecasting model using a hybrid CNN-LSTM architecture. In this approach, a one-dimensional convolutional neural network (CNN) extracts local patterns from the input sequence of data and Long Short-Term Memory (LSTM) network is used to learn long-term dependencies. Rather than predicting one step at a time, the model performs **direct multi-step forecasting**, generating a full sequence of future values in parallel. The model forecasts weekly energy consumption over a 10-week horizon and achieves a **Mean Absolute Percentage Error (MAPE) of 12%**, corresponding to an overall accuracy of **88%**, demonstrating strong forecasting performance for this type of long-range, multi-step prediction. The accompanying plot shows how closely the predicted values track actual energy usage over the forecast period.
